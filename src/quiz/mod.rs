...
pub fn parse_question_line(line: &str) -> Result<QuizQuestion> {
    ...
    if parts.len() != 9 {
        bail!(
            "Invalid format: expected 9 fields separated by |, got {}",
            parts.len()
        );
    }
    ...
    let answer_texts: Vec<String> = parts[1..5].iter().map(|s| s.trim().to_string()).collect();

    // Validate all answer texts are non-empty
    for (idx, text) in answer_texts.iter().enumerate() {
        if text.is_empty() {
            bail!(
                "Answer option {} cannot be empty",
...
mod tests {
    ...
    fn test_parse_empty_explanation() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Explanation cannot be empty"));
    }
    ...
    fn test_parse_empty_subdomain() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Subdomain cannot be empty"));
    }
...
"#;
    ...
    fn test_load_sample_file() {
        ...
        let content = fs::read_to_string(sample_path).expect("Failed to read sample question file");

        let questions = parse_question_file(&content).expect("Failed to parse sample questions");
