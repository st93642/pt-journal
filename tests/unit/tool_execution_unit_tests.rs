...
mod new_ai_tools_tests {
    ...
    fn test_new_ai_tools_available() {
        ...
        // Check PyRIT
        assert!(
            manifest.iter().any(|entry| entry.id == "pyrit"),
            "PyRIT should be available"
        );
        ...
        // Check PentestGPT
        assert!(
            manifest.iter().any(|entry| entry.id == "pentestgpt"),
            "PentestGPT should be available"
        );
        let pentestgpt_entry = manifest
            .iter()
            .find(|entry| entry.id == "pentestgpt")
            .unwrap();
        ...
        assert!(
            manifest.iter().any(|entry| entry.id == "nemo_guardrails"),
            "NeMo Guardrails should be available"
        );
        let nemo_entry = manifest
            .iter()
            .find(|entry| entry.id == "nemo_guardrails")
            .unwrap();
    ...
    fn test_new_ai_tools_have_instructions() {
        ...
        assert!(
            tool_instructions::has_tool("pyrit"),
            "PyRIT should have instructions"
        );
        assert!(
            tool_instructions::has_tool("pentestgpt"),
            "PentestGPT should have instructions"
        );
        assert!(
            tool_instructions::has_tool("nemo_guardrails"),
            "NeMo Guardrails should have instructions"
        );
        ...
        assert!(
            pentestgpt_doc.is_some(),
            "PentestGPT instructions should be loadable"
        );

        let nemo_doc = tool_instructions::get_instructions("nemo_guardrails");
        assert!(
            nemo_doc.is_some(),
            "NeMo Guardrails instructions should be loadable"
        );
    }
    ...
    fn test_ai_tools_category_count() {
        ...
        assert_eq!(
            ai_tools_count, 9,
            "Should have exactly 9 AI & LLM Security tools, found {}",
            ai_tools_count
        );
    }
    ...
    fn test_total_tool_count_increased() {
        ...
        assert_eq!(
            manifest.len(),
            229,
            "Should have exactly 229 tools after adding 3 new AI tools"
        );
    }
}
