...
fn test_default_app_model() {
    ...
    assert_eq!(model.session().phases.len(), 32); // Updated: added Tutorial 10 (advanced_reconnaissance_techniques)
}

fn test_session_creation() {
    let session = Session::default();
    assert!(!session.name.is_empty());
    assert_eq!(session.phases.len(), 32); // Updated: added Tutorial 10 (advanced_reconnaissance_techniques)
}
