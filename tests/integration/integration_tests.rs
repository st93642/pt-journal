...
fn test_default_app_model() {
    ...
    assert_eq!(model.session().phases.len(), 28); // Updated: added Tutorial 5 (server-side-attacks-csrf-ssrf-file-inclusion)
}

fn test_session_creation() {
    let session = Session::default();
    assert!(!session.name.is_empty());
    assert_eq!(session.phases.len(), 28); // Updated: added Tutorial 5 (server-side-attacks-csrf-ssrf-file-inclusion)
}
