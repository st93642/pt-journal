...
fn test_default_app_model() {
    ...
    assert_eq!(model.session().phases.len(), 23); // Updated: migrated all tutorial modules to JSON
}

fn test_session_creation() {
    let session = Session::default();
    assert!(!session.name.is_empty());
    assert_eq!(session.phases.len(), 23); // Updated: migrated all tutorial modules to JSON
}
